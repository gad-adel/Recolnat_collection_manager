...
import static org.recolnat.collection.manager.api.domain.enums.imports.ImportFieldFormatEnum.DATE_AS_STRING;
...
public class ImportIdentificationFileChecker {

    public static final Map<String, ImportFieldFormatEnum> FIELDS_TO_CHECK = Map.ofEntries(
            Map.entry("DETERMINATION", DATE_AS_STRING)
    );
    ...
    private static void logTimes(long timeElapsed) {
        if (log.isInfoEnabled()) {
            log.info("checkFileData : {} ms", timeElapsed / 1000000);
            log.info("advancedChecks total : {} ms", TIMES.get("advancedChecks") / 1000000);
            log.info("\tblockingChecks total : {} ms", TIMES.get("blockingChecks") / 1000000);
            log.info("\t\tcheckUserRights total : {} ms", TIMES.get("checkUserRights") / 1000000);
            log.info("\t\tcheckIfCollectionExists total : {} ms", TIMES.get("checkIfCollectionExists") / 1000000);
            log.info("\t\tcheckRequiredValues total : {} ms", TIMES.get("checkRequiredValues") / 1000000);
            log.info("\t\tcheckFieldsFormat total : {} ms", TIMES.get("checkFieldsFormat") / 1000000);
            log.info("\t\tcheckDateField total : {} ms", TIMES.get("checkDateField") / 1000000);
            log.info("\tnonBlockingChecks total : {} ms", TIMES.get("nonBlockingChecks") / 1000000);
        }
    }
    ...
    public ImportCheckIdentificationResponseDTO checkFileData(MultipartFile determinationFile, UUID institutionId, ImportCheckSpecimen specimensChecks) {
        initTimes();
        long start = System.nanoTime();
        var response = new ImportCheckIdentificationResponseDTO();
    ...
    private void advancedChecks(List<String[]> lines, ImportCheckIdentificationResponseDTO response, UUID institutionId,
