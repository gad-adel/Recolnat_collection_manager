...
import io.recolnat.api.SpecimenIntegrationAsDraftApi;
...
@Slf4j
public class SpecimenIntegrationAsDraftResource implements SpecimenIntegrationAsDraftApi {

    private final SpecimenIntegrationService specimenIntegrationService;
    private final SpecimenMapper specimenMapper;

    @Override
    public ResponseEntity<Void> addSpecimenAsDraft(SpecimenIntegrationRequestDTO specimenIntegrationRequestDTO) {
        ...
        try {
            ...
            final var id = specimenIntegrationService.addAsDraft(specimen);

            var uri = UriComponentsBuilder.fromHttpUrl("http://localhost:8080/v1/specimens/" + id.getSpecimenId())
                    .build().toUri();
            return ResponseEntity.created(uri).header("specimenId", id.getSpecimenId().toString()).build();
