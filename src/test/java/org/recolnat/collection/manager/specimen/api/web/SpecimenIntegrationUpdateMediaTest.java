...
@ActiveProfiles("int")
@Slf4j
public class SpecimenIntegrationUpdateMediaTest extends AbstractResourceElasticTest {
    ...
    void updateSpecimen_whenMultipleCoversSent_shouldKeepOnlyFirst() throws Exception {
        ...
        when(authenticationService.findUserAttributes()).thenReturn(UserAttributes.builder().ui(UUID.randomUUID().toString()).role(ADMIN_INSTITUTION.name()).institution(1).collections(List.of(UUID.fromString(COLLECTION_ID))).build());
