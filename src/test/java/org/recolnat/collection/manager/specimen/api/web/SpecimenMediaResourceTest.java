...
import org.recolnat.collection.manager.connector.api.impl.MediathequeServiceImpl;
...
class SpecimenMediaResourceTest extends AbstractResourceElasticTest {
    ...
    @MockBean
    protected MediathequeServiceImpl mediathequeService;
    @MockBean
    ...
    void addMedia_should_be_ok() throws Exception {
        ...
        doNothing().when(checkAttribut).checkUserAuthAttributesForAnyRole(UUID.fromString(COLLECTION_ID));
        ...
        when(mediathequeService.savePicture(oneFile)).thenReturn(new ResponseEntity<>(media, HttpStatus.CREATED));
