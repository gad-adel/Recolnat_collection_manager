...
class SpecimenIntegrationResourceTest extends AbstractResourceElasticTest {
    ...
    @ParameterizedTest()
    @ValueSource(strings = {"REVIEW", "VALID", "DRAFT"})
    @DisplayName("Integration test duplicate specimen")
    void givenSpecimen_whenDuplicate_thenReturnSpecimenDuplicate(String statusValue) throws Exception {
        // Given - precondition or setup
        var body = objectMapper.readValue(specimenData.getInputStream(), SpecimenIntegrationRequestDTO.class);
        ...
        // When - action or the behaviour
        mvc.perform(
                        post("/v1/specimens/{specimenId}/duplicate/{collectionTargetId}?operation={statusValue}",
                                SPECIMEN_ID, COLLECTION_ID, OperationTypeDTO.fromValue(statusValue))
                                .with(opaqueToken()
                                        .attributes(att -> att.put("sub", UID_CONST))
                                        .authorities(new SimpleGrantedAuthority("ADMIN_INSTITUTION")))
                                .with(user("test")
